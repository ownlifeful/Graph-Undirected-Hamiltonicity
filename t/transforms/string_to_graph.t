...
foreach my $test ( @tests ) {
    ...
    is( scalar( $G->vertices() ), $test->{expected_vertex_count}, "Preserved number of vertices. [$test->{label}]");
    is( scalar( $G->edges() ), $test->{expected_edge_count}, "Preserved number of edges. [$test->{label}]");
    if ( $test->{preserved} ) {
        is( "$G", $test->{input_graph_text}, "Graph to string is the same as string to graph. [$test->{label}]");
    } else {
        isnt( "$G", $test->{input_graph_text}, "Graph to string is NOT the same as string to graph. [$test->{label}]");
    }
}
