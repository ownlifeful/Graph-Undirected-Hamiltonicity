...
use Graph::Undirected::Hamiltonicity::Transforms qw(&string_to_graph &get_required_graph);
...
foreach my $test ( @tests ) {
    ...
    is( "$required_graph", $test->{expected_required_graph_text}, "Got the expected required graph.");

    foreach my $edge_ref ( $required_graph->edges() ) {
        is( $output_graph->get_edge_attribute(@$edge_ref,'required'), 1, "the edge has been marked required.");
